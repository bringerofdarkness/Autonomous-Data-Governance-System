...
from app.models import DocumentMetadata, DocumentStatus
...
def summarize_pii(detected_pii: list[dict[str, str]]) -> dict[str, int]:
    pii_type_counts = {}
...
def process_document_task(document_id: str) -> dict[str, str]:
    try:
        document = DocumentMetadata.objects.get(id=uuid.UUID(document_id))
    ...
    try:
        ...
        # 3. Initialize and run LangGraph
        ...
        graph_result = graph.invoke(initial_state, config=graph_config)

        # 4. Check for workflow interrupt (HITL review required)
        if "__interrupt__" in graph_result:
            ...
            create_document_audit_log(
                ...
                message=document.conflict_summary or "Conflict Agent completed before HITL pause.",
                extra_data={
        ...
        create_document_audit_log(
            ...
            actor_user_id=None,
            action="DOCUMENT_CONFLICT_CHECKED",
            message=document.conflict_summary or "Conflict Agent completed with no summary.",
            extra_data={
                "conflict_found": document.conflict_found,
                "conflict_summary": document.conflict_summary,
                "conflict_checked_at": (
        ...
        if graph_result.get("requires_admin_approval"):
            ...
            create_document_audit_log(
                ...
                message=graph_result.get("critic_feedback", "Document requires admin approval."),
                extra_data={
        ...
        return {
            ...
            "requires_admin_approval": str(graph_result.get("requires_admin_approval", False)),
            "risk_level": graph_result.get("risk_level", "LOW"),
        }
